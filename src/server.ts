...
process.on("uncaughtException", (error) => {
  process.stderr.write(
    `[FATAL] Uncaught exception: ${error instanceof Error ? error.stack || error.message : error}\n`,
  );
  process.exit(1);
});
...
process.on("unhandledRejection", (reason) => {
  process.stderr.write(
    `[ERROR] Unhandled rejection (server continuing): ${reason instanceof Error ? reason.stack || reason.message : reason}\n`,
  );
});
